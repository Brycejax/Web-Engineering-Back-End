...
router.post('/', function(req, res){
    if(!req.body.orders || !req.body.loyalty_points || !req.body.pkgTracking)
    ...
            db.collection(USERHISTORY_COLLECTION_NAME)
...
router.put('/:id', function(req, res){
    ...
            const db = connection.db(DB_NAME);
            db.collection(USERHISTORY_COLLECTION_NAME).updateOne({"_id" : objectId(req.params.id)},                 
...
router.put('/:id', function(req, res){
    if(!req.params.id || req.params.id.length === 0)
            return res.status(400).send({message: "User History ID is required."})
        ...
        client.connect(function(err, connection){
            ...
            const db = connection.db(DB_NAME);
            assert.equal(null, err);
                db.collection(USERHISTORY_COLLECTION_NAME).deleteOne({"_id": objectId(id)}, function(del_err, result){
                    if (del_err){
                        return res.status(500).send({message: "Something went wrong."})
                    }
                    assert.equal(null, err);
